...
/*	ArrowUColor    = gdImageColorAllocate( im,    0, 0xFF, 0 ); */
    ...
    if (GDC_grid || GDC_ticks || GDC_yaxis) { /* grid lines & y label(s) */
        float tmp_y      = lowest;
                ...
                for (i = 0; i < num_points; ++i)
                    if (uvals[CLOSESET + j * 3][i] != GDC_NOVALUE) {                                                                    /* all HLC have vert line */
                        ...
                        if (GDC_HLC_style & GDC_HLC_DIAMOND) {
                            ...
                            cd[0].y = PY(uvals[CLOSESET + j * 3][i]);
                            cd[1].x = PX(i);
                            cd[1].y = PY(uvals[CLOSESET + j * 3][i]) + hlf_hlccapwdth;
                            cd[2].x = PX(i) + hlf_hlccapwdth;
                            cd[2].y = PY(uvals[CLOSESET + j * 3][i]);
                            cd[3].x = PX(i);
                            cd[3].y = PY(uvals[CLOSESET + j * 3][i]) - hlf_hlccapwdth;
