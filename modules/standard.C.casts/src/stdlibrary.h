...
#define CONCEPT_FUNCTION_IMPL(function_name, parameters_count)                                CONCEPT_DLL_API CONCEPT_ ## function_name CONCEPT_API_PARAMETERS { static char *error = 0; if (PARAMETERS->COUNT != parameters_count) { error =#function_name " takes " #parameters_count " parameters."; return (void *)error; } LOCAL_INIT; char *func_name =#function_name;
...
#define T_STRING(parameter_index, fname)                                 \
    __INTERNAL_PARAMETER_DECL(char *, bind, parameter_index);            \
    __INTERNAL_PARAMETER_DECL(NUMBER, bind_len, parameter_index);        \
    error =#fname ": parameter " #parameter_index " should be a string"; \
    GET_CHECK_BUFFER(parameter_index, __INTERNAL_PARAMETER(bind, parameter_index), __INTERNAL_PARAMETER(bind_len, parameter_index), /*__PARAM_ERR_MSG(func_name,parameter_index,"string")*/ error);

#define T_NUMBER(parameter_index, fname)                                 \
    __INTERNAL_PARAMETER_DECL(NUMBER, bind, parameter_index);            \
    __INTERNAL_PARAMETER_DECL(INTEGER, bind_len, parameter_index);       \
    error =#fname ": parameter " #parameter_index " should be a number"; \
    GET_CHECK_NUMBER(parameter_index, __INTERNAL_PARAMETER(bind, parameter_index), /*__PARAM_ERR_MSG(func_name,parameter_index,"number")*/ error);
