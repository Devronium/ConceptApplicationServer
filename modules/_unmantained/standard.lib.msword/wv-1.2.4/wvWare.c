...
do_version(void) {
    ...
    GET_printf_HANDLER() ("wvWare %s\n", "1.2.4");
}

static void
do_help(void) {
    do_version();
    GET_printf_HANDLER() ("Usage: wvWare [OPTION...] filename.doc\n");
    GET_printf_HANDLER() ("\nCommon Options:\n");
    GET_printf_HANDLER() ("  -x --config=config.xml\tSpecify an output filter to use\n");
    GET_printf_HANDLER() ("  -c --charset=charset\t\tSpecify an iconv charset encoding\n");
    GET_printf_HANDLER() ("  -p --password=password\tSpecify password for encrypted\n\t\t\t\tWord Documents\n");
    GET_printf_HANDLER() ("  -d --dir=dir\t\t\tDIR\n");
    GET_printf_HANDLER() ("  -b --basename=name\t\tUse name as base name of image files\n");
    GET_printf_HANDLER() ("  -a --auto-eps=fmt\t\tQuery support for conversion of fmt to eps\n");
    GET_printf_HANDLER() ("  -s --suppress=fmt\t\tDon't convert fmt to eps\n");
    GET_printf_HANDLER() ("  -X --xml\t\t\tXML ouput\n");
    GET_printf_HANDLER() ("  -1 --nographics\t\tno 0x01 graphics output\n");
    GET_printf_HANDLER() ("  -v --version\t\t\tPrint wvWare's version number\n");
    GET_printf_HANDLER() ("  -? --help\t\t\tPrint this help message\n");
    GET_printf_HANDLER()
    ...
    GET_printf_HANDLER() ("Authors:\nDom Lachowicz (dominicl@seas.upenn.edu)\n");
    GET_printf_HANDLER() ("Caolan McNamara (original author)\nVisit http://www.wvware.com\n");
}
...
main(int argc, char **argv) {
    ...
    if (optind >= argc) {
        GET_fprintf_HANDLER() (stderr, "No file name given to open\n");
        return -1;
    }
...
char *figure_name(wvParseStruct *ps) {
    ...
    if (b_name == 0) {
        GET_fprintf_HANDLER() (stderr, "error: unable to create basename!");
        exit(1);
    }
    ...
    if (f_name) {
        ...
        GET_fprintf_HANDLER() (stderr, "error: unable to create filename!");
        exit(1);
    }
