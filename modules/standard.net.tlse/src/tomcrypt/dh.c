...
int dh_export_pqY(unsigned char *pbuf, unsigned long *plen, unsigned char *gbuf, unsigned long *glen, unsigned char *Ybuf, unsigned long *Ylen, dh_key *key) {
   unsigned long len;
   int err;
   void *p, *g;

   LTC_ARGCHK(pbuf  != NULL);
   LTC_ARGCHK(plen  != NULL);
   LTC_ARGCHK(gbuf  != NULL);
   LTC_ARGCHK(glen  != NULL);
   LTC_ARGCHK(Ybuf  != NULL);
   LTC_ARGCHK(Ylen  != NULL);
   LTC_ARGCHK(key   != NULL);
