...
CONCEPT_FUNCTION_IMPL_MINMAX_PARAMS(Murmur64, 1, 2)
    T_STRING(Murmur64, 0)
    uint32_t seed = 0;
    if (PARAMETERS_COUNT > 1) {
        T_NUMBER(Murmur3, 1)
    ...
    char buffer[17];
    buffer[0] = 0;
    snprintf(buffer, sizeof(buffer), "%llx", MurmurHash64A(PARAM(0), PARAM_LEN(0), seed));
...
//---------------------------------------------------------------------------
CONCEPT_FUNCTION_IMPL(ChaCha20Init, 1)
    T_STRING(ChaCha20Init, 0)
    u_int kbits = PARAM_LEN(0) * 8;
    switch (kbits) {
        case 128:
        case 256:
            break;
        default:
            return (void *)"ChaCha20Init: Invalid key size. Key must be 128 or 256 bits long";
    }
    struct chacha_ctx *ctx = (struct chacha_ctx *)malloc(sizeof(struct chacha_ctx));
    if (ctx) {
        memset(ctx, 0, sizeof(struct chacha_ctx));
        chacha_keysetup(ctx, (u8 *)PARAM(0), kbits);
    }
    RETURN_NUMBER((SYS_INT)ctx);
END_IMPL
//---------------------------------------------------------------------------
CONCEPT_FUNCTION_IMPL(ChaCha20, 2)
    T_HANDLE(ChaCha20, 0)
    T_STRING(ChaCha20, 1)
    chacha_ctx *ctx = (chacha_ctx *)(SYS_INT)PARAM(0);
    SYS_INT len = PARAM_LEN(1);
    if (len > 0) {
        chacha_ctx *ctx = (chacha_ctx *)(SYS_INT)PARAM(0);
        char *buf = NULL;
        CORE_NEW(len + 1, buf);
        if (buf) {
            chacha_encrypt_bytes(ctx, (u8 *)PARAM(1), (u8 *)buf, len);
            SetVariable(RESULT, -1, buf, len);
        } else {
            RETURN_STRING("");
        }
    } else {
...
END_IMPL
//---------------------------------------------------------------------------
CONCEPT_FUNCTION_IMPL(ChaCha20Release, 1)
    T_HANDLE(ChaCha20Release, 0)
    chacha_ctx *ctx = (chacha_ctx *)(SYS_INT)PARAM(0);
    free(ctx);
    SET_NUMBER(0, 0);
    RETURN_NUMBER(0);
