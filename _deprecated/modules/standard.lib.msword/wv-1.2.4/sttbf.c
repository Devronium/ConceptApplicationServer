...
wvListSTTBF(STTBF *item) {
    ...
    if (item->s8strings != NULL) {
        for (i = 0; i < item->nostrings; i++)
            GET_fprintf_HANDLER() (stderr, "string is %s\n", item->s8strings[i]);
    } else if (item->u16strings != NULL) {
        for (i = 0; i < item->nostrings; i++) {
            GET_fprintf_HANDLER() (stderr, "string is ");
            letter = item->u16strings[i];
            while ((letter != NULL) && (*letter != 0))
                GET_fprintf_HANDLER() (stderr, "%c", *letter++);
    ...
    if (item->extradata != NULL) {
        for (i = 0; i < item->nostrings; i++)
            for (j = 0; j < item->extradatalen; j++)
                GET_fprintf_HANDLER() (stderr, " %x ", item->extradata[i][j]);
        GET_fprintf_HANDLER() (stderr, "\n");
    }
}
