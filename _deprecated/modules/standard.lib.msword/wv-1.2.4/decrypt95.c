...
wvCrack95(wvParseStruct *ps) {
    ...
    GET_fprintf_HANDLER() (stderr, "reallen is %x\n", reallen);
    ...
    if ((block1[0] == 0) && (block2[0]) && (reallen < 0xff)) {
        GET_fprintf_HANDLER() (stderr, "good\n");
        key[8] = reallen ^ block2[0];
    } else
        GET_fprintf_HANDLER() (stderr, "crap\n");
    ...
    GET_fprintf_HANDLER() (stderr, "%x %x %x %x\n", block2[0], block2[1], block2[1],
                           block2[0]);
    ...
    for (i = 0; i < 16; i++)
        GET_fprintf_HANDLER() (stderr, "key %d is %x\n", i, key[i]);
    return 0;
}
#endif
