...
wvHandleCommandField(wvParseStruct *ps, char *command) {
    ...
    while ((token = strtok(NULL, "\t, "))) {
        ...
        switch (s_Tokens[tokenIndex].m_type) {
            case FC_HYPERLINK:
                token = strtok(NULL, "\"\" ");
                GET_printf_HANDLER() ("<a href=\"%s\">", token);
                break;
            ...
            case FC_EMBED:
                ...
                /*	GET_printf_HANDLER() ("<!--%s-->", token);
                 */break;

            case FC_PAGEREF:
                token = strtok(NULL, "\"\" ");
                GET_printf_HANDLER() ("<a href=\"#%s\" %s>", token, xml_slash);
                break;
            ...
            case FC_DateTimePicture:
                ...
                if (wvHandleDateTimePicture(datestr, 4096, token, &mytime)) {
                    /* GET_printf_HANDLER() ("%s", datestr); */ /* prefer to print out the text that follows the spec char */
                } else
                    wvError(
            ...
            case FC_DATEINAME:
                if (ps->filename) {
                    GET_printf_HANDLER()("%s", ps->filename);
                }
                ret = 1;
                break;
