...
main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        GET_fprintf_HANDLER() (stderr, _("Usage: wvSummary doc1 [... docN]\n"));
        return 1;
    }
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (!input) {
            GET_fprintf_HANDLER() (stderr, _("Problem with getting metadata from %s:%s\n"),
        ...
        GET_fprintf_HANDLER() (stdout, _("Metadata for %s:\n"), argv[i]);
