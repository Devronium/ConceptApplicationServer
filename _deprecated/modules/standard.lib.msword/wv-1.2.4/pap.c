...
wvAddPAPXFromBucket(PAP *apap, UPXF *upxf, STSH *stsh, wvStream *data) {
    ...
    while (i < upxf->cbUPX - 2) {
        GET_fprintf_HANDLER() (stderr, "%x (%d) ", *(upxf->upx.papx.grpprl + i),
    ...
    /*
       while (i < upxf->cbUPX-2)
     */
    while (i < upxf->cbUPX - 4) {
        /* the end of the list is at -2, but there has to be a full sprm of
           len 2 as well */
...
wvAssembleSimplePAP(wvVersion ver, PAP *apap, U32 fc, PAPX_FKP *fkp, wvParseStruct *ps) {
    ...
    if ((papx) && (papx->cb > 2)) {
        ret = 1;
#ifdef SPRMTEST
        GET_fprintf_HANDLER() (stderr, "cbUPX is %d\n", papx->cb);
        for (i = 0; i < papx->cb - 2; i++)
            GET_fprintf_HANDLER() (stderr, "%x ", papx->grpprl[i]);
        GET_fprintf_HANDLER() (stderr, "\n");
#endif
        upxf.cbUPX           = papx->cb;
