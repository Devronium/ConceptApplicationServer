...
VariableDATA *ClassCode::ExecuteDelegate(PIFAlizator *PIF, INTEGER i, struct CompiledClass *Owner, const RuntimeOptimizedElement *OE, ParamList *FORMAL_PARAM, VariableDATA **SenderCTX, INTEGER CLSID, INTEGER LOCAL_CLSID, VariableDATA **LOCAL_THROW, SCStack *PREV, int result_id THREAD_CREATION_LOCKS) const {
    ...
    *LOCAL_THROW = NULL;
    ...
    if (pMEMBER_i) {
        ...
        RESULT = pMEMBER_i->Execute(PIF, this->CLSID, Owner, FORMAL_PARAM, SenderCTX, *LOCAL_THROW, PREV, (result_id > 0) ? SenderCTX[result_id] : NULL, 0, NULL, 0, thread_lock);
#else
        RESULT = pMEMBER_i->Execute(PIF, this->CLSID, Owner, FORMAL_PARAM, SenderCTX, *LOCAL_THROW, PREV, (result_id > 0) ? SenderCTX[result_id] : NULL, 0, NULL, 0);
#endif
        UNSTACK;
