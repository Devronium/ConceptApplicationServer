...
int GetVariableByName(int operation, void **VDESC, void **CONTEXT, int Depth, char *VariableName, char *buffer, int buf_size, void *PIF, void *STACK_TRACE) {
    if (operation == -1) {
        ...
        while (STACK_TRACE) {
            ...
            cstack += ":";
            if (first) {
                cstack += AnsiString((D_LONG_TYPE)CM->_DEBUG_STARTLINE);
                first   = 0;
            } else
                cstack += AnsiString((D_LONG_TYPE)((SCStack *)STACK_TRACE)->line);
            cstack     += "\n";
            STACK_TRACE = (SCStack *)((SCStack *)STACK_TRACE)->PREV;
        }
        INTERNAL_UNLOCK(((PIFAlizator *)PIF))
        buffer [buf_size - 1] = 0;
        strncpy(buffer, cstack.c_str(), buf_size - 1);
        buffer [cstack.Length()] = 0;
        return -1;
    }
    ...
    if ((i >= 0) && (i < Depth)) {
        ...
        if (((VariableDATA *)CONTEXT [i])->TYPE == VARIABLE_NUMBER) {
            if (operation == 0) {
                strncpy(buffer, AnsiString(((VariableDATA *)CONTEXT [i])->NUMBER_DATA).c_str(), buf_size);
            } else
            if (operation == 1) {
                ((VariableDATA *)CONTEXT [i])->NUMBER_DATA = AnsiString(buffer).ToFloat();
            }
        ...
        if (((VariableDATA *)CONTEXT [i])->TYPE == VARIABLE_CLASS) {
            if (operation == 0) {
                strncpy(buffer, ((CompiledClass *)((VariableDATA *)CONTEXT [i])->CLASS_DATA)->GetClassName().c_str(), buf_size);
            }
            if (!point) {
                CLEAN_VARS;
                ...
                switch (VD->TYPE) {
                    ...
                    case VARIABLE_NUMBER:
                        if (operation == 0) {
                            strncpy(buffer, AnsiString(VD->NUMBER_DATA).c_str(), buf_size);
                        } else
                        if (operation == 1) {
                    ...
                    case VARIABLE_CLASS:
                        if (operation == 0) {
                            strncpy(buffer, ((CompiledClass *)VD->CLASS_DATA)->GetClassName().c_str(), buf_size);
                        }
                        CLEAN_VARS;
                        return VD->TYPE;
                        break;
                    ...
                    case VARIABLE_ARRAY:
                        if (operation == 0) {
                            strncpy(buffer, ((Array *)VD->CLASS_DATA)->ToString(), buf_size);
                        }
                        CLEAN_VARS;
                        return VD->TYPE;
                        break;
