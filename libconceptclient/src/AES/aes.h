...
public:
    ...
    void Encrypt(const unsigned char *datain, unsigned char *dataout, unsigned int numBlocks, BlockMode mode = AES::CBC);
    ...
    void Decrypt(const unsigned char *datain, unsigned char *dataout, unsigned int numBlocks, BlockMode mode = AES::CBC);

    int bsize;
    unsigned char buffer[64];    // initialization vector buffer
private:
