...
static TSKID_t   __SEMHH_START_TASK(void *f, void *arg) {
    ...
    if (__SEMHH_TASK_TABLE == NULL) {
        ...
        void *p = malloc(lg);
        if (!p)
            abend("strtsk", -1, "alloc task table, not enough storage");
    ...
    if (__SEMHH_TASK_NUM == __SEMHH_TASK_MAX) {         /* task table is full, realloc ... */
        ...
        if (!p)
            abend("strtsk", -1, "re-alloc task table, not enough storage");
...
static TSKID_t   __SEMHH_DEL_TASK_BY_NUM(int n) {
    ...
    if (m < 0)
        abend("waitsk", n, "DEL_TASK_BY_NUM, bad n");
    if (m > 0)
        memcpy(p, p + 1, m * sizeof(TSKID_t));
    --__SEMHH_TASK_NUM;
    return cid;
}
...
static TSKID_t   __SEMHH_DEL_TASK_BY_ID(TSKID_t cid) {
    ...
    for (n = 0; n < __SEMHH_TASK_NUM; ++n)
        if (cid == __SEMHH_TASK_TABLE[n])
            return __SEMHH_DEL_TASK_BY_NUM(n);
    return 0;
}
...
static TSKID_t   __SEMHH_WAIT_SINGLE_TASK(TSKID_t cid, int *prc) {
    if (cid == 0)
        return __SEMHH_WAIT_SINGLE_TASK(cid, prc);
...
static TSKID_t   __SEMHH_DEL_TASK_BY_NUM(int n) {
    ...
    if (m < 0)
        abend("del_tsk_by_num", n, "__SEMHH_TASK_NUM - n - 1 < 0");
    if (m > 0)
...
static TSKID_t   __SEMHH_WAIT_MULTIPLE(int *prc) {
    ...
    semp(__SEMHH_TASK_TABLE_MUTEX);
    for (n = 0; n < __SEMHH_TASK_NUM; ++n)
        if (__SEMHH_TASK_TABLE[n].fl == __SEMHH_TASK_ENDED)
            goto found_it;
...
static TSKID_t   __SEMHH_WAIT_SINGLE_TASK(TSKID_t cid, int *prc) {
    ...
    if (cid == 0)
        return __SEMHH_WAIT_MULTIPLE(prc);
    semp(__SEMHH_END_TASK);
...
static TSKID_t   __SEMHH_EXIT_TASK(int rc) {
    ...
    for (n = 0; n < __SEMHH_TASK_NUM; ++n)
        if (cid == __SEMHH_TASK_TABLE[n].id)
            goto found_it;
